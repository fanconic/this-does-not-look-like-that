...
def setup_test_image(idx):
    ...
    if not os.path.isdir(OUT+'datasets/cub200_cropped/' + typ + '_cropped/' + file_name.split('/')[0]) :
        os.mkdir(OUT+'datasets/cub200_cropped/' + typ + '_cropped/' + file_name.split('/')[0])
...
def setup_data(num_classes=200):
    ...
    for i in tqdm(images.index):
        ...
        assert image_id == bb.loc[i, 'image_id'] and image_id == train_test_split.loc[i, 'image_id']
        typ = 'train' if is_train else 'test'
        if not os.path.isdir(OUT+'datasets/cub200_cropped/' + typ + '_cropped/' + file_name.split('/')[0]) :
            os.mkdir(OUT+'datasets/cub200_cropped/' + typ + '_cropped/' + file_name.split('/')[0])
