...
def similarity_score(
    ...
    assert (
        loc != None
    ), "please provide the locations in feature map as a list of dict (x, y)"
    ...
    if act_loc is not None:
        ...
        return (
            torch.mean(sim) - torch.mean(act_sim),
            local_max_sim,
            global_max_sim,
...
def pgd(
    ...
    upsampled_activation_pattern = cv2.resize(
        activation_pattern, dsize=(img_size, img_size), interpolation=cv2.INTER_CUBIC
    )
